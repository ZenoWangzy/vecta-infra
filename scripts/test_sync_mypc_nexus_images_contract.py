...
HERMES_SOURCE = (
    "127.0.0.1:8083/nousresearch/hermes-agent:v2026.8.19"
...
def main() -> None:
    ...
    assert f"manifest index {HERMES_SOURCE} -> 127.0.0.1:8082/{HERMES_TARGET}" in output
    assert (
        "+ skopeo copy --all --preserve-digests --src-tls-verify=false "
        f"--dest-tls-verify=false docker://{HERMES_SOURCE} "
        f"docker://127.0.0.1:8082/{HERMES_TARGET} "
        in output
    )
    assert f"+ docker pull {HERMES_SOURCE} " not in output
    assert f"+ docker tag {HERMES_SOURCE} 127.0.0.1:8082/{HERMES_TARGET} " not in output
